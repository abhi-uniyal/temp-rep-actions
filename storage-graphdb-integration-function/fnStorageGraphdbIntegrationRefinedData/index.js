...
const folderPath = process.env.AZURE_STORAGE_FOLDER_PATH_REFINED_DATA;
...
module.exports = async function (context, myBlob) {
    ...
    switch (blobName) {
      case 'refined-product-data-1.ttl':
        context.log(`filename ::::${fileName} `);
        //curlCommand = `curl -X PUT -d @${fileName} -H "Content-Type: application/x-turtle;charset=UTF-8" "http://${GRAPHDB_HOST}:${GRAPHDB_PORT}/repositories/${GRAPHDB_REPO}/rdf-graphs/service?graph=https:%2F%2Fknowledge.ikea.net%2Fproduct-range-data-graph%2Frefined-product-data-1&role=context" -H "Authorization: Basic ${BASIC_AUTH}"`;
        context.log(`CURL command called.....`);
        break;

      case 'refined-product-data-2.ttl':
        context.log(`filename ::::${fileName} `);
        //curlCommand = `curl -X PUT -d @${fileName} -H "Content-Type: application/x-turtle;charset=UTF-8" "http://${GRAPHDB_HOST}:${GRAPHDB_PORT}/repositories/${GRAPHDB_REPO}/rdf-graphs/service?graph=https:%2F%2Fknowledge.ikea.net%2Fproduct-range-data-graph%2Frefined-product-data-2&role=context" -H "Authorization: Basic ${BASIC_AUTH}"`;
        context.log(`CURL command called.....`);
        break;
